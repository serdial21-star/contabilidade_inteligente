...
Arquitetura nativa: `config.js` (configuração pública), `core.js` (sessão), `oidc-client.js` (OIDC/PKCE), `api-client.js` (rede centralizada), `mock-provider.js` (fonte sintética), `dashboard-service.js` (Minha Visão), `work-context.js` (empresa/competência e política local de revisão), `operational-service.js` (Empresas/Documentos), `intelligence-service.js` (Fiscal/Financeiro) e `app.js` (bootstrap/shell). Nenhuma senha é recebida. O access token permanece somente em memória; `sessionStorage` guarda apenas state/verifier PKCE efêmeros e os remove no callback. `localStorage` contém preferências minimizadas de layout e contexto de trabalho, sempre revalidadas contra as empresas e permissões atuais; nenhum XML, valor, nome empresarial, CNPJ ou token é persistido.
