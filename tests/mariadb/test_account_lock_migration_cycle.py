...
@pytest.fixture
def account_lock_migration_engine(migration_lab_at_revision: object) -> object:
    with migration_lab_at_revision('head') as engine:  # type: ignore[operator]
        yield engine


def test_account_lock_migration_downgrade_and_reupgrade(account_lock_migration_engine: object) -> None:
    url = _config.get('MARIADB_MIGRATION_DATABASE_URL')
    assert url
    original = os.environ.get('DATABASE_URL')
    try:
        with account_lock_migration_engine.connect() as connection:  # type: ignore[attr-defined]
