...
class SqlAlchemyBankingRepository:
 ...
 def list_accounts(self,t:UUID,c:UUID)->tuple[BankAccount,...]:
  rows=self._session.scalars(select(BankAccountModel).where(BankAccountModel.tenant_id==t,BankAccountModel.company_id==c).order_by(BankAccountModel.bank_name,BankAccountModel.bank_id,BankAccountModel.created_at));return tuple(_account(row) for row in rows)
 def get_account(self,t:UUID,c:UUID,account_id:UUID)->BankAccount|None:
  row=self._session.scalar(select(BankAccountModel).where(BankAccountModel.tenant_id==t,BankAccountModel.company_id==c,BankAccountModel.id==account_id));return _account(row) if row else None
 def find_account(self,t:UUID,c:UUID,identity:str)->BankAccount|None:
  row=self._session.scalar(select(BankAccountModel).where(BankAccountModel.tenant_id==t,BankAccountModel.company_id==c,BankAccountModel.external_identity==identity,BankAccountModel.status=='ACTIVE'));return _account(row) if row else None
 def find_account_any(self,t:UUID,c:UUID,identity:str)->BankAccount|None:
 ...
 def add_account(self,x:BankAccount)->None:self._session.add(BankAccountModel(**asdict(x)))
 def update_account(self,x:BankAccount)->None:
  row=self._session.scalar(select(BankAccountModel).where(BankAccountModel.tenant_id==x.tenant_id,BankAccountModel.company_id==x.company_id,BankAccountModel.id==x.id))
  if row is None: return
  for field in ('bank_id','branch_id','account_number','account_type','currency_code','external_identity','bank_name','nickname','status','updated_at'):setattr(row,field,getattr(x,field))
 ...
 def add_statement(self,x:BankStatement)->None:self._session.add(BankStatementModel(**asdict(x)))
 ...
 def add_transaction(self,x:BankTransaction)->None:self._session.add(BankTransactionModel(**asdict(x)))
def _account(x:BankAccountModel)->BankAccount:return BankAccount(x.id,x.tenant_id,x.company_id,x.bank_id,x.branch_id,x.account_number,x.account_type,x.currency_code,x.external_identity,x.created_at,x.bank_name,x.nickname,x.status,x.updated_at)
